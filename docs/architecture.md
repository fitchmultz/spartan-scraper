...
- Web UI is the primary interactive AI surface for preview, template generation, template debugging, render-profile generation, render-profile debugging, pipeline-JS generation, pipeline-JS debugging, research refinement, export-shape authoring for recurring exports, saved-result transform authoring, and job-form controls.
...
- The shared `internal/aiauthoring` service now owns bounded AI preview, template generation, template debugging, render-profile generation, render-profile debugging, pipeline-JS generation, pipeline-JS debugging, research refinement, export shaping, saved-result transform generation, validation retries, live rechecks, and optional request-scoped multimodal image context (uploaded/pasted reference images plus fetched screenshots) so REST, Web, CLI, and MCP do not drift.
- TUI stays focused on local operations and inspection. It may display AI-related metadata already attached to jobs, but it is not a first-class surface for prompt-heavy AI workflows such as preview, template generation, template debugging, render-profile generation, render-profile debugging, pipeline-JS generation, pipeline-JS debugging, research refinement, export shaping, saved-result transform authoring, or agent sessions.
