...
- The shared `internal/aiauthoring` service now owns bounded AI preview, template generation, template debugging, render-profile generation, render-profile debugging, pipeline-JS generation, pipeline-JS debugging, research refinement, export shaping, validation retries, live rechecks, and optional request-scoped multimodal image context (uploaded/pasted reference images plus fetched screenshots) so REST, Web, CLI, and MCP do not drift.
