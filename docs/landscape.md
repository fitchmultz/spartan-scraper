...
Interface symmetry is intentional, not absolute. Spartan aims for shared capability where it improves operator outcomes, but it does not force every interface to host every workflow. In particular, AI preview, template authoring, request-scoped multimodal image-assisted authoring, AI template debugging, AI render-profile generation, AI render-profile debugging, AI pipeline-JS generation, AI pipeline-JS debugging, bounded research-result refinement, bounded recurring export-shape authoring, and proxy-pool runtime inspection plus request-scoped proxy selection belong in the Web UI, API, CLI, and MCP rather than the TUI, which remains focused on local inspection and job operations. Those authoring and runtime-inspection flows now share the same bounded implementation path across the supported non-TUI surfaces instead of drifting into Web-only or API-only features.
