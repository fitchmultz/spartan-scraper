...
- Track the temporary transitive Go dependency overrides in `go.mod` and remove them as soon as upstream parent modules absorb those newer tags, so dependency freshness stops depending on local replace directives.
