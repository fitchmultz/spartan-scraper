...
- Missing default `DATA_DIR/proxy_pool.json` never blocks startup, while explicit custom `PROXY_POOL_FILE` overrides still surface configuration mistakes instead of silently pretending proxy-backed execution is active.
