...
`spartan watch check <id>` now persists a watch check record before returning. The CLI prints the guided inspection summary immediately, including the saved `checkId`, and `spartan watch history <id>` can reopen that history later. Use `--check-id <check-id>` to inspect a single saved check, or `--json` to emit the canonical `{ check }` / `{ checks, total, limit, offset }` envelopes instead of the guided text view.
...
`GET /v1/jobs/{id}/results` is the raw persisted-results inspection surface (`jsonl`/`json` plus jsonl pagination). `POST /v1/jobs/{id}/export` is the canonical direct export/download surface for `json`, `jsonl`, `md`, `csv`, and `xlsx` with optional bounded `shape` or `transform` controls. `GET /v1/watch` now uses the canonical paginated `{ watches, total, limit, offset }` envelope. `POST /v1/watch/{id}/check` now persists and returns the canonical `{ check }` inspection envelope immediately, while `GET /v1/watch/{id}/history` and `GET /v1/watch/{id}/history/{checkId}` expose the same guided watch inspection model across surfaces. Latest watch artifacts still live at `GET /v1/watch/{id}/artifacts/{artifactKind}`, and historical artifact snapshots are available through `GET /v1/watch/{id}/history/{checkId}/artifacts/{artifactKind}` without exposing host-local paths.
...
The watch tools expose the stored watch-management workflow already available in the other primary operator surfaces: `watch_list`, `watch_get`, `watch_create`, `watch_update`, `watch_delete`, `watch_check`, `watch_check_history`, and `watch_check_get`. `watch_list` now accepts optional `limit` and `offset` arguments and returns the canonical `{ watches, total, limit, offset }` envelope. `watch_check` returns the canonical `{ check }` inspection envelope used by the Web UI and watch-history routes, `watch_check_history` returns `{ checks, total, limit, offset }`, and `watch_check_get` returns one `{ check }` inspection envelope by watch id + check id.
