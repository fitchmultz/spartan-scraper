...
func (s *Server) handleOAuthInitiate(w http.ResponseWriter, r *http.Request) {
	...
	stateStr, codeVerifier, err := oauthStore.CreateOAuthState(req.ProfileName, req.RedirectURI, oauthConfig.UsePKCE)
	if err != nil {
		writeError(w, r, err)
		return
	}
	...
	if oauthConfig.UsePKCE {
		_, challenge, method, err := auth.GeneratePKCE()
		if err != nil {
			writeError(w, r, err)
			return
		}
		...
		// Update the stored state with the verifier
		if loadedState, ok, _ := oauthStore.LoadState(stateStr); ok {
			loadedState.CodeVerifier = codeVerifier
			oauthStore.SaveState(loadedState)
		}
	}

	// Build authorization URL
	authURL, err := auth.BuildAuthorizationURL(*oauthConfig, stateStr, codeChallenge, codeChallengeMethod)
	if err != nil {
		writeError(w, r, err)
		return
	}
...
func (s *Server) handleOAuthRevoke(w http.ResponseWriter, r *http.Request) {
	...
	if profile.OAuth2.RevokeURL != "" {
		if err := auth.RevokeOAuth2Token(r.Context(), profile.OAuth2.RevokeURL, profile.OAuth2.ClientID, profile.OAuth2.ClientSecret, existingToken.AccessToken, "access_token"); err != nil {
			...
			slog.Error("failed to revoke token at provider", "error", err)
		}
	}
