...
func TestHostMatchesAnyPattern_EmptyPatterns(t *testing.T) {
	matches := hostmatch.HostMatchesAnyPattern("example.com", []string{})
	if matches {
...
func TestHostMatchesAnyPattern_ExactMatch(t *testing.T) {
	matches := hostmatch.HostMatchesAnyPattern("example.com", []string{"example.com", "other.com"})
	if !matches {
		t.Error("expected true for exact match")
	}
	matches = hostmatch.HostMatchesAnyPattern("example.com", []string{"other.com", "test.com"})
	if matches {
...
func TestHostMatchesAnyPattern_WildcardPrefix(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := hostmatch.HostMatchesAnyPattern(tt.host, tt.patterns); got != tt.want {
...
func TestHostMatchesAnyPattern_WildcardSuffix(t *testing.T) {
	tests := []struct {
		...
		{"prefix matches", "example.org", []string{"example.*"}, true},
		{"prefix with co.uk matches", "example.co.uk", []string{"example.*"}, true},
		{"non-matching prefix", "other.org", []string{"example.*"}, false},
		{"partial prefix no match", "myexample.org", []string{"example.*"}, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := hostmatch.HostMatchesAnyPattern(tt.host, tt.patterns); got != tt.want {
...
func TestHostMatchesAnyPattern_CaseInsensitive(t *testing.T) {
	tests := []struct {
		...
		{"host uppercase", "EXAMPLE.COM", []string{"example.com"}, true},
		{"pattern uppercase", "example.com", []string{"EXAMPLE.COM"}, true},
		{"both uppercase", "EXAMPLE.COM", []string{"EXAMPLE.COM"}, true},
...
func TestHostMatchesAnyPattern_TrimsWhitespace(t *testing.T) {
	tests := []struct {
		...
		{"host with whitespace", "  example.com  ", []string{"example.com"}, true},
		{"pattern with whitespace", "example.com", []string{"  example.com  "}, true},
		{"both with whitespace", "  example.com  ", []string{"  example.com  "}, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := hostmatch.HostMatchesAnyPattern(tt.host, tt.patterns); got != tt.want {
