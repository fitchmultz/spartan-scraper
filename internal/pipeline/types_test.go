...
func TestHostFromURL_InvalidURL(t *testing.T) {
	tests := []struct {
		...
		{"not a url", ""},      // Invalid URL returns empty string
		{"http://", ""},        // Missing host returns empty string
		{"://example.com", ""}, // Invalid scheme returns empty string
		{"EXAMPLE.COM", "example.com"},
	}
	for _, tt := range tests {
		t.Run(tt.url, func(t *testing.T) {
...
func TestHostFromURL_WithSchemeParsingFailure(t *testing.T) {
	tests := []struct {
		...
		// Note: Go's url.Parse is lenient with <>, so this parses successfully
		// but the hostname extraction may vary by implementation
		{"http://invalid<>url.com", "invalid<>url.com"},
		{"https://invalid url.com", ""}, // Space in host returns empty
	}
	for _, tt := range tests {
		t.Run(tt.url, func(t *testing.T) {
			host := HostFromURL(tt.url)
