/**
 * Purpose: Present the modal AI helper that proposes bounded export shapes from representative saved results.
 * Responsibilities: Collect operator instructions, call the export-shape AI endpoint, surface generated shape details, and require explicit apply confirmation.
 * Scope: Modal presentation and request state for export-shape assistance only.
 * Usage: Mount from export and results workflows when operators request AI-assisted shape suggestions.
 * Invariants/Assumptions: The modal never auto-applies generated shapes, representative job IDs stay operator-provided, and AI-unavailable states must stay self-explanatory.
 */
...
export function AIExportShapeAssistant({
  ...
  const aiCapability = describeAICapability(
    aiStatus,
    "Configure export shaping manually in the schedule form.",
  );
                ...
                {state.response.inputStats ? (
                  <div style={{ marginTop: 12 }}>
                    ...
                    <div className="job-list">
                      <div className="job-item">
                        Field options{" "}
                        {state.response.inputStats.fieldOptionCount}
                      </div>
                      <div className="job-item">
